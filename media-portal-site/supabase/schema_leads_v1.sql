...
create table if not exists public.leads (
  ...
  threeb_id text not null,
  threeb_business_id text not null,
  name text not null,
  email text not null,
  phone text,
  business_name text,
...
update public.leads set threeb_id = coalesce(threeb_id, 'UNKNOWN_3B_ID') where threeb_id is null;
update public.leads set threeb_business_id = coalesce(threeb_business_id, 'UNKNOWN_3B_BIZ_ID') where threeb_business_id is null;
...
alter table public.leads alter column threeb_id set not null;
alter table public.leads alter column threeb_business_id set not null;
...
create index if not exists idx_leads_threeb_id on public.leads (threeb_id);
create index if not exists idx_leads_threeb_business_id on public.leads (threeb_business_id);
