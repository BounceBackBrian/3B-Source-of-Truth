...
├─ app/
│  ├─ page.tsx
│  ├─ (public)/{services,portfolio,about,contact,pricing,start}
...
- Auth pages collect 3B ID + 3B Business ID and prefill `/start` via browser storage to keep intake identity consistent.
...
- Public `/start` form captures `threeb_id` and `threeb_business_id` and posts both to `POST /api/leads` for lead-level identity mapping.
...
- Apply migration: `supabase/migrations/20260303_leads_add_threeb_identity.sql` (adds not-null 3B identity fields on leads).
...
## 16) Main page troubleshooting
- If `/` shows a blank/minimal page, ensure deployment root is `media-portal-site` (not parent folder).
- Legacy static paths now redirect into App Router:
  - `/index.html` → `/`
  - `/business.html` → `/start`
  - `/support.html` → `/contact`
- Always start locally from this directory:
  - `cd media-portal-site && npm install && npm run dev`
