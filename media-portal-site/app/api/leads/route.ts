...
async function sendLeadNotifications(input: {
  ...
  const slackMsg =
    ...
    `Name: ${input.name}\n` +
    `3B ID: ${input.threeb_id}\n` +
    `3B Business ID: ${input.threeb_business_id}\n` +
    `Need: ${input.need}\n` +
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.5">
      ...
      <b>3B ID:</b> ${escapeHtml(input.threeb_id)}<br/>
      <b>3B Business ID:</b> ${escapeHtml(input.threeb_business_id)}<br/>
...
export async function POST(req: NextRequest) {
  try {
    ...
    const threebId = body.threeb_id?.trim() || '';
    const threebBusinessId = body.threeb_business_id?.trim() || '';
    ...
    if (!threebId || !threebBusinessId || !name || !email || !message || !need || !timeline) {
    ...
    const { error } = await supabase.from('leads').insert({
      threeb_id: threebId,
      threeb_business_id: threebBusinessId,
      name,
      email,
